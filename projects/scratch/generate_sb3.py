#!/usr/bin/env python3
"""
Generate a Scratch 3.0 (.sb3) Grade Calculator — STARTER project.
Programming Merit Badge — Requirement 5c

This generates the base project with TODO challenge comments
for Scouts to work on.
...
B_SAY_AVG = uid(); B_SAY_GR = uid(); B_SAY_MSG = uid(); B_SAY_BYE = uid()
...
mkblk(B_WELCOME, "looks_sayforsecs",
      inputs={"MESSAGE": str_lit("Welcome to the Grade Calculator! 📊"),
              "SECS": num_lit(2)},
...
# Grade if/else chain (using not < for >=)
for letter, threshold, grade, msg, parent_id, nxt_if, nxt_else, set_gr, set_mg in [
    ("A", "90", "A", "Amazing work! 🌟", B_SET_AVG, B_SAY_AVG, B_IF_B, B_SET_GR_A, B_SET_MG_A),
]:
    pass  # Handled individually below for clarity
...
mkblk(B_SAY_MSG, "looks_sayforsecs",
      ...
      parent=B_SAY_GR, nxt=B_SAY_BYE)

mkblk(B_SAY_BYE, "looks_sayforsecs",
      inputs={"MESSAGE": str_lit("Thanks for using the Grade Calculator! 🎓"), "SECS": num_lit(2)},
      parent=B_SAY_MSG)

# ---- TODO CHALLENGE COMMENTS ----

add_comment(B_IF_A,
            "TODO Challenge #1: Add plus/minus grades!\n"
            "Ex: 90-93 = A-, 94-96 = A, 97-100 = A+\n"
            "Add more if/else blocks inside each grade section.",
            x=480, y=-100, w=250, h=120)

add_comment(B_SAY_BYE,
            "TODO Challenge #2: Add sound effects!\n"
            "Play a 'win' sound for A/B grades\n"
            "and a different sound for C/D/F.\n"
            "Use 'play sound until done' from Sound.",
            x=480, y=50, w=250, h=120)

add_comment(B_REPEAT,
            "TODO Challenge #3: Track the highest\n"
            "and lowest scores!\n"
            "Create 'highestScore' and 'lowestScore'\n"
            "variables and update them in this loop.",
            x=480, y=-50, w=250, h=120)

add_comment(B_SAY_MSG,
            "TODO Challenge #4: Make the sprite react!\n"
            "Change the sprite's costume based on\n"
            "the grade (happy for A, sad for F).\n"
            "Add costumes in the Costumes tab first!",
...
project = {
    "targets": [
        ...
        {
            "isStage": False, "name": "Grade Calculator",
...
print(f"  {len(blocks)} blocks, {len(comments)} TODO comments, {len(VARS)} variables")
